...
def upgrade() -> None:
    ...
    project_type_enum = postgresql.ENUM(
        'FOREST_RESTORATION', 'AFFORESTATION', 'MANGROVE_CONSERVATION',
        'WETLAND_CONSERVATION', 'BIODIVERSITY_CONSERVATION', 'OTHER',
        name='project_type_enum'
    )
    project_type_enum.create(op.get_bind())

    project_status_enum = postgresql.ENUM(
        'PLANNING', 'ACTIVE', 'MONITORING', 'COMPLETED', 'ARCHIVED',
    ...
    op.create_table('projects',
        ...
        sa.Column('status', postgresql.ENUM(name='project_status_enum', create_type=False), nullable=False),
    ...
    op.create_table('sites',
        ...
        sa.Column('name', sa.String(length=255), nullable=False),
        sa.Column('geometry', geoalchemy2.types.Geometry(geometry_type='POLYGON', srid=4326, from_text='ST_GeomFromEWKT', name='geometry', nullable=False, spatial_index=False)),
        sa.Column('area_hectares', sa.Numeric(precision=12, scale=4), nullable=False),
        sa.Column('centroid', geoalchemy2.types.Geometry(geometry_type='POINT', srid=4326, from_text='ST_GeomFromEWKT', name='geometry', nullable=True, spatial_index=False)),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
    ...
    op.create_table('site_analytics',
        ...
        sa.Column('tree_cover_percentage', sa.Numeric(precision=5, scale=2), nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
        sa.CheckConstraint('biodiversity_score >= 0 AND biodiversity_score <= 100', name='chk_biodiversity_score_range'),
        sa.CheckConstraint('carbon_tonnes >= 0', name='chk_carbon_tonnes_positive'),
        sa.CheckConstraint('tree_cover_percentage >= 0 AND tree_cover_percentage <= 100', name='chk_tree_cover_percentage_range'),
        sa.CheckConstraint('vegetation_index >= 0 AND vegetation_index <= 1', name='chk_vegetation_index_range'),
